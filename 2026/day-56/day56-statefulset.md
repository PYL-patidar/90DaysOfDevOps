...
volumeClaimTemplate:  
volumeClaimTemplates is used in a StatefulSet to create storage automatically for each Pod.  
When a new Pod starts, Kubernetes will automatically create a Persistent Volume Claim (PVC) for that Pod.  
So every Pod gets its own storage.  
